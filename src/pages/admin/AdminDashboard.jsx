...
const AdminDashboard = () => {
  ...
  const processWeeklyData = (weeklyRiskTrend) => {
    const dataWithDates = Object.entries(weeklyRiskTrend).map(
      ([date, score]) => ({
      ...
      <div className="relative bg-gray-50 dark:bg-gray-900 min-h-screen pb-4">
        <div className="py-6 px-6 md:px-10">
          <div className="mb-8 animate-fade-in">
            <h1 className="page-header mb-2 text-gray-800 dark:text-gray-100">
              HR Dashboard
            </h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {cardData.map((item, index) => (
              ...
              >
                <div
                  className={`h-10 w-10 rounded-sm ${item.title === "Critical Cases" ? "bg-red-100 dark:bg-red-900/30" : "bg-green-100 dark:bg-green-900/30"} flex justify-center items-center`}
                >
                  {item.sign}
                </div>
                <p className="text-gray-400 dark:text-gray-300 font-semibold text-lg">
                  {item.title}
                </p>
