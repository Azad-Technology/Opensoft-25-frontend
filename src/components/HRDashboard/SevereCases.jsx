...
const SevereCases = ({ criticalCases }) => {
  ...
  const currentPosts =
    searchedEmployees?.slice(indexOfFirstPost, indexOfLastPost) || [];
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4 px-4 py-4 bg-secondary">
            <div className="text-sm text-muted-foreground">
              ...
              <span className="font-medium">
                {Math.min(indexOfLastPost, searchedEmployees.length)}
              </span>{" "}
              of <span className="font-medium">{searchedEmployees.length}</span>{" "}
              employees
            </div>
            <div className="flex items-center">
              ...
              <div className="flex space-x-1 mx-1">
                {(() => {
                  const totalPages = Math.ceil(
                    (searchedEmployees?.length || 0) / postsPerPage,
                  );
                  const pageNumbers = [];
              ...
              <button
                ...
                disabled={
                  currentPage ===
                  Math.ceil((searchedEmployees?.length || 0) / postsPerPage)
                }
                className={`flex items-center justify-center w-8 h-8 rounded-md ${
                  currentPage ===
                  Math.ceil((searchedEmployees?.length || 0) / postsPerPage)
